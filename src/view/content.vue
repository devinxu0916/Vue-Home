  ...
  <div class="wrapper">
    ...
    <div class="main">
      ...
      <ul id="comments-main">
        ...
        <li v-for="(item, index) in data.replies" :key="item.id">
          <img :src="item.author.avatar_url" alt="">
          <strong>{{ item.author.loginname }}</strong>
          <!--点赞-->
          <span class="comments-peopele-ups">
             <mu-icon @click="good(index)" value="thumb_up" :size="16" color="#a9a9a9" />{{ item.ups.length }}
          </span>
          <!--时间-->
          <span class="comments-peopele-time">{{ item.create_at | formatDate }}</span><br>
          <!--评论图标按钮-->
          ...
          <div v-if="accessToken" v-show="item.reply_show" class="comment-reply">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      hasCollected () {
        if (this.accessToken) {
          axios.get('https://www.vue-js.com/api/v1/user/' + this.loginName).then((res) => {
